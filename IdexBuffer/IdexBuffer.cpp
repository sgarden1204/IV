...
HRESULT InitIB()
{
	...
	if (FAILED(g_pd3dDevice->CreateIndexBuffer(triangle * sizeof(MYINDEX), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pIB, NULL)))
	{//여기
		return E_FAIL;
	}
...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, vertex, 0, triangle);
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	if (SUCCEEDED(InitD3D(hWnd)))
	{
		...
		fscanf_s(file, "%d", &vertex);
		fscanf_s(file, "%d", &triangle);
		...
		for (int i = 0; i < vertex; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				{
				case 0:
					fscanf_s(file, "%f", &vertices[i].x);
					break;
				case 1:
					fscanf_s(file, "%f", &vertices[i].y);
					break;
				case 2:
					fscanf_s(file, "%f", &vertices[i].z);
					break;
				}
			}
		}
		...
		for (int i = 0; i < triangle; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				{
				case 0:
					fscanf_s(file, "%d", &indices[i]._0);
					break;
				case 1:
					fscanf_s(file, "%d", &indices[i]._1);
					break;
				case 2:
					fscanf_s(file, "%d", &indices[i]._2);
					break;
				}
			}
		}
